...
async function aggregateCycleData(): Promise<CycleData[]> {
  ...
  for (let i = 0; i < cycleFiles.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (filename) => {
      const cycleNumber = parseInt(filename.replace('cycle-', '').replace('.json', ''));
...
async function aggregateUserData(): Promise<{ totalUsers: number; totalStacked: number; averageStacked: number; mostActiveUsers: UserData[] }> {
  ...
  const batchSize = 10;
  for (let i = 0; i < Math.min(userFiles.length, 100); i += batchSize) { // Limit to first 100 users for performance
    ...
    const batchPromises = batch.map(async (filename) => {
      ...
      if (data && data.stackingHistory) {
        const userTotalStacked = data.stackingHistory.reduce((sum: number, cycle: any) => sum + (cycle.amount || 0), 0);
        const cyclesParticipated = data.stackingHistory.length;
        const lastActiveDate = data.stackingHistory[data.stackingHistory.length - 1]?.date || '';
...
async function fetchRewardData(): Promise<any[]> {
  ...
  const rewardFiles = await fetchGitHubDirectory('packages/home/content/rewards');
  const rewardData: any[] = [];
  
  // Process first 10 reward files
  const limitedFiles = rewardFiles.slice(0, 10);
  for (const filename of limitedFiles) {
    const data = await fetchJsonFile(`packages/home/content/rewards/${filename}`);
    if (data) {
      rewardData.push({
...
async function generateAnalytics(): Promise<AnalyticsResponse> {
  ...
  console.log(`Generated analytics: ${cycleData.length} cycles, ${userData.totalUsers} users`);
